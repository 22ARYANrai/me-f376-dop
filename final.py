...
# Try loading the CSV file and printing its columns to check the actual column names
df = pd.read_csv('data21.csv')
print(df.columns)  # Add this line to see the actual column names

# If the CSV does not have column names, you can manually specify them like this:
# df = pd.read_csv('data21.csv', header=None, names=["velocity", "feed", "ktc", "krc"])

# Verify that the required columns are present
if "velocity" not in df.columns or "feed" not in df.columns:
    raise KeyError("The columns 'velocity' and 'feed' are not found in the CSV file.")
